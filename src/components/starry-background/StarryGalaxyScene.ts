...
export function createStarryControlPanel(): void {
  ...
  fGalaxy
    .add(starryGuiState, 'camZoom', 1.5, 25, 0.01)
  ...
  fSupernova
    .add(starryGuiState.supernova, 'opacity', 0, 1, 0.01)
    .name(tf('starryBg.opacity') ?? '透明度')
    .onChange(apply)
  ...
  fDisplay
    ...
    .onChange((v) => useStarryUiStore.getState().setInkEnabled(Boolean(v)))
  const hideProxy = { on: useStarryUiStore.getState().starryHidden }
  fDisplay
    .add(hideProxy, 'on')
    .name(tf('starryBg.hideStarry'))
    .onChange((v) => {
